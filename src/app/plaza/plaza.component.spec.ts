...
describe('PlazaComponent', () => {
  ...
  it('should render the view correctly', () => {
    ...
    expect(buttons[6].textContent?.trim()).toBe('Confessional');
    expect(buttons[6].getAttribute('routerLink')).toBe('/confessional');
    expect(buttons[7].tagName).toBe('A');
    expect(buttons[7].textContent?.trim()).toBe('Council Chambers');
    expect(buttons[7].getAttribute('routerLink')).toBe('/council');
  });
});
