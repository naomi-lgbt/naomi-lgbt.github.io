...
(async () => {
  ...
  const rawArt = await fetch('https://sfo3.digitaloceanspaces.com/naomi-cdn?prefix=naomi/art');
  ...
  const rawEmotes = await fetch('https://sfo3.digitaloceanspaces.com/naomi-cdn?prefix=naomi/emotes');
  ...
  const rawGames = await fetch('https://sfo3.digitaloceanspaces.com/naomi-cdn?prefix=naomi/games');
  ...
  const rawOutfits = await fetch('https://sfo3.digitaloceanspaces.com/naomi-cdn?prefix=naomi/outfit');
  const outfitText = await rawOutfits.text();
  const outfitData = await parser.parseStringPromise(outfitText);
  const outfits = outfitData.ListBucketResult.Contents.map(i => i.Key[0].split('/')[2]).filter(el => el);

  const rawPoses = await fetch('https://sfo3.digitaloceanspaces.com/naomi-cdn?prefix=naomi/koikatsu');
  ...
  const rawTattoos = await fetch('https://sfo3.digitaloceanspaces.com/naomi-cdn?prefix=naomi/ref/tattoos');
  const tattoosText = await rawTattoos.text();
  const tattoosData = await parser.parseStringPromise(tattoosText);
  const tattoos = tattoosData.ListBucketResult.Contents.map(i => i.Key[0].split('/')[3]).filter(el => el);
